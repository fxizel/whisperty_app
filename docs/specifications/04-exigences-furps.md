...
| **CO-18** | La diarisation est **désactivée par défaut** (`speaker_diarization.enabled: false`) : l'utilisateur l'active explicitement ; sans modèle en cache, un téléchargement **opt-in** est proposé (jamais silencieux). | M | UC-18 | Installation neuve : la réunion utilise la distinction par source ; l'activation de la diarisation demande confirmation si le modèle est absent. |
| **CO-19** | Le modèle de diarisation est **téléchargeable hors-ligne** (procédure documentée, analogue UC-14) et **indisponible en exe figé** sans bundling préalable (`can_download: false` si pas de pip). | S | UC-18, UC-14 | En dev, le modèle se met en cache localement ; en exe figé sans modèle embarqué, la diarisation est refusée avec message explicite. |
