...
*Vérification* = critère d'acceptation observable. *Source* = UC et/ou référence documentaire (cf. `00`).
...
| **FR-03** | Transcrire l'audio **localement** via faster-whisper, modèle configurable (`base`/`small`/`medium`/`large-v3`), `beam_size` paramétrable. | M | UC-01 | La transcription s'effectue sans accès réseau (modèle en cache) et respecte le modèle de `config.yaml`. |
...
| **FR-07** | Appliquer un **dictionnaire** : *hotwords* (biais de reconnaissance) + corrections `mauvais => bon` post-transcription. | S | UC-04 | Un hotword listé est mieux reconnu ; une correction définie est appliquée au texte final. |
...
| **FR-12** | **Transcrire en direct** une sortie audio (loopback) en continu, écriture au fil de l'eau dans un fichier horodaté, copie + archivage à l'arrêt. | S | UC-09 | La capture loopback produit un transcript croissant ; à l'arrêt le texte est copié et archivé (source `live`). |
| **FR-13** | **Transcrire une réunion** en capturant micro **+** sortie système simultanément ; par défaut **distinction par locuteur** (entrelacement chronologique `Moi`/`Interlocuteurs`), sinon mixage ; export `.txt`/`.md` horodaté, **sans injection**. | S | UC-10 | `distinguish_speakers: true` produit des lignes `[MM:SS] Moi/Interlocuteurs` ; `false` produit une voix mixée ; un fichier export est créé. |
...
| **FR-16** | Refléter l'**état** par l'icône tray (gris=prêt, rouge=enreg., orange=transcription, bleu=live, vert=réunion, violet=assistant) et émettre des **notifications** (best-effort). | M | UC-01, UC-09…11 | Chaque transition change la couleur/infobulle ; les fins d'opération notifient l'utilisateur. |
| **FR-17** | Charger l'ensemble du comportement depuis un **unique `config.yaml`** + un `dictionary.txt`. | M | UC-12 | Toute clé documentée modifie le comportement après relance ; absence de fichier → valeurs par défaut. |
| **FR-18** | **Précharger** le modèle au démarrage (en tâche de fond) afin d'accélérer la première dictée. | S | UC-14 | Au lancement, l'état passe brièvement en `PROCESSING` le temps du chargement, puis `IDLE`. |
...
| **US-02** | Toutes les actions secondaires sont accessibles depuis un **menu tray** clair (dictée, live, réunion, assistant, import, historique, config, quitter). | M | UC-07…12 | Le menu clic droit expose chaque action ; les actions indisponibles sont désactivées (grisées). |
...
| **RE-04** | L'**ordre de verrouillage** est respecté (`_lock` → `_op_lock`, jamais l'inverse ; le callback PortAudio ne prend aucun verrou) pour exclure tout interblocage. | M | UC-01, UC-09…11 | Aucun blocage observé lors d'arrêts concurrents ; les modes exclusifs s'arrêtent sans `join()` sous verrou. |
...
| ID | Exigence | Prio | Source | Vérification |
|----|----------|------|--------|--------------|
| **PE-01** | La transcription doit être **exploitable sur CPU** avec le modèle par défaut (`medium`/`int8`) ; l'accélération CUDA est possible pour réduire la latence. | M | UC-01, UC-15 | Une dictée courte est transcrite sur CPU sans configuration GPU ; CUDA réduit le temps observé. |
...
| **CO-02** | `transcription.local_files_only: true` **par défaut** ; en mode hors-ligne, `HF_HUB_OFFLINE`/`TRANSFORMERS_OFFLINE` sont posés. | M | UC-14, R-3 | Avec le modèle en cache, l'app fonctionne sans réseau dès le défaut. |
| **CO-03** | Le mode IA n'accepte **que des endpoints locaux** (`localhost`/`127.0.0.1`/`::1`) ; tout hôte distant est refusé. | M | UC-06, UC-11 | Un endpoint distant est rejeté ; seul un endpoint local est accepté. |
